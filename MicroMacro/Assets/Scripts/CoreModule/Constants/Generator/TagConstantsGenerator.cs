...
namespace CoreModule.Constants.Generator
{
    public class TagConstantsGenerator
    {
        ...
        private const string subClassName = "Handle";
        ...
        private static void Generate()
        {
            ...
            using (builder.CreateBlockScope($"namespace {nameof(CoreModule.Constants)}"))
            {
                ...
                using (builder.CreateBlockScope($"public sealed class {className}"))
                {
                    ...
                    {
                        builder.NewLine($"public static readonly string {tag.field} = \"{tag.field}\";");
                    }
                    
                    builder.NewLine();

                    using (builder.CreateBlockScope($"public sealed class {subClassName}"))
                    {
                        foreach ((string field, string real) tag in tags)
                        {
